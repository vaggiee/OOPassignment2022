...
int main() {
	...
	cout << "Choose a team: W or w for werewolves and V or v for vampires"<<endl
		<<"If you got a magic potion,by pressing left shift you can heal your team!" << endl;
	cin >> tteam;
	//Checks users input
	...
	while (g) {
		...
		for (int time = 0; time < 24,g==1; time++) {
			// Clear the screen
			system("cls");
			...
			if(mya.get_team()=='V' || mya.get_team() == 'v')cout << "You are with the vampires";
			else{ cout << "Your team is werewolves"; }
			map1->printmap(x, y);
			//Geets users input
			switch (getKey()) {
			case VK_LEFT:
